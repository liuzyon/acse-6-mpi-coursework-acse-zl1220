...
void copy_boundry(vector< vector<double> > &grid, double** recv_array, bool recv_left, bool recv_top, bool recv_right, bool recv_bottom)
{
	int imax = grid.size();
	int jmax = grid[0].size();
	...
	if (recv_right)
	{
		...
		for (int i = 1; i < imax-1; i++)
		{
			grid[i][jmax-1] = recv_array[i][jmax-1];
		}
	}
	...
	if (recv_bottom)
	{
		...
		for (int i = 1; i < jmax-1; i++)
		{
			grid[imax-1][i] = recv_array[imax-1][i];
		}
	}

}

int imax = 10, jmax = 10;	//棋盘大小
//进程负责的grid
...
int main(int argc, char *argv[])
{
	...
	// cout << id << " 进程结构的行index: " << id_row << " 列index: " << id_column << endl;

	int grid_rows, grid_columns; //平均
	//分配每个进程的domain
	allocate_grid(grid, new_grid, old_grid, imax, jmax, grid_rows, grid_columns);

	// cout << id << " 棋盘行数" << grid.size() << " 棋盘列数" << grid[0].size() << endl;
	...
	while (t < 0.05)
	{
		...
		//准备发送数据, 注意，data_array是去除ghost layer的domain，而gird是包括ghost layer的，要注意对应
		for (int i = 0; i < m; i++)
			for (int j = 0; j < n; j++)
				data_array[i][j] =(double) grid[i+1][j+1];
		...
		//将边界值赋值
		copy_boundry(grid, recv_array, recv_left, recv_top, recv_right, recv_bottom);
